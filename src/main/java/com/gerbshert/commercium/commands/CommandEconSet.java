            ...
            if (commandRan) {
                switch (args.length) {
                    case 1:
                        ...
                        break;
                    case 2:
                        Bank.setBalance(sender.getName(), Bank.getBalance(args[1]) + amount);
                        if (sender instanceof EntityPlayer) {
                            ((EntityPlayer) sender).addChatMessage(new TextComponentString("[Commercium]: Balance of " + args[1] + " set to " + amount));
                        }
                        sender.getEntityWorld().getPlayerEntityByName(args[1]).addChatMessage(new TextComponentString("[Commercium]: Your balance was set to " + amount));
                        ;
                        break;
                    default:
                        if (sender instanceof EntityPlayer) {
                            ((EntityPlayer) sender).addChatMessage(new TextComponentString("[Commercium]: Try '/help set'"));
                        }
                        ;
                        break;
                }
